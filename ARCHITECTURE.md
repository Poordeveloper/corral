...
| **Installation** | the directory an installed Corral's three executables are siblings in — `~/Applications/Corral.app/Contents/MacOS` or `~/.local/share/corral/bin` under the user's home as Corral resolves it — plus the `~/.local/bin/corral` link into it. Recognized from the running executable's resolved pathname and nothing else; a build under `target/` is not one. What `uninstall` removes, and never the Corral root unless told to |
