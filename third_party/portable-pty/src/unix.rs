...
pub fn close_random_fds() {
    ...
    if let Ok(dir) = std::fs::read_dir("/dev/fd") {
        ...
        for fd in fds {
            unsafe {
                // CORRAL PATCH 1 (third_party/portable-pty/CORRAL_PATCHES.md):
                // never close a close-on-exec descriptor here. The kernel
                // closes it on a successful exec anyway, while closing it
                // early destroys the pipe libstd uses to report exec failure
                // to the parent — turning every post-fork execve failure into
                // a child-side abort the parent reads as an ordinary exit.
