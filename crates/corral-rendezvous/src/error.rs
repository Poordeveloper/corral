...
pub enum RendezvousError {
    ...
    /// A runtime directory exists but is not exactly user-private. Corral's
    /// runtime state is private to its owner by construction.
    RuntimeDirectoryNotPrivate { path: PathBuf, mode: u32 },
    /// A runtime directory belongs to another account.
    RuntimeDirectoryNotOwned {
        path: PathBuf,
        owner: u32,
        expected: u32,
    },
...
impl fmt::Display for RendezvousError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::RuntimeDirectoryNotOwned {
                path,
                owner,
                expected,
            } => write!(
                f,
                "the runtime directory {} belongs to uid {owner}, not to uid {expected}; Corral \
                 keeps its runtime state private to the account that owns it",
                path.display()
            ),
