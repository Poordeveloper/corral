...
### 2026-09-11 — Q7: the bundle identifier is `com.carriez.corral`

Founder, unprompted, while the bundle was still unreleased:
「请把com.poordevelper.corral改成com.carriez.corral」.

What changes: `packaging/macos/Info.plist.in` and the key the package smoke
asserts. What it reaches: the notification probe this file specifies under
Q22 uses "the real bundle identity" — that identity is now
`com.carriez.corral`, and `m1-notifications` seals the mechanism against
it. The Q7 body above is the transcript of that day and is left as it was;
this section is the authority. Why now is free: TCC keys authorization on
the bundle identifier, so a change after the notification probe would
discard every grant the probe established, and nothing has been released
under the old identifier. The freeze holds from here — the identifier
becomes load-bearing at the first published release, not before.
